...
Los analisis se encuentran en [**este repositorio**](https://github.com/adbpatagonia/seal_lice).
...
No se pudo computar la condicion relativa de `r nrow(leo[is.na(rel_cond)])` focas, dado que no se registro su peso. De ellas, `r nrow(leo[is.na(rel_cond) & Lice > 0])` focas (`r pull(leo[is.na(rel_cond) & Lice > 0, .(Sp)])`) tenian `r pull(leo[is.na(rel_cond) & Lice > 0, .(Lice)])` piojo cada una .            
...
El efecto de la condicion  sobre el cuantil 50% de abundancia resulto ser no significativo, <i>p-value</i>: `r round(parameters::model_parameters(m.lice.abun.cond.nozero.qr)$p[2], 2)`, aunque el coeficiente fue negativo, <i>pendiente</i>: `r round(parameters::model_parameters(m.lice.abun.cond.nozero.qr)$Coefficient[2], 2)`.  Es decir, la condicion corporal  parece afectar negativamente la abundancia alta de piojos en focas leopardo (aunque no es significativo estadisticamente). Notese que solo hay 4 datos que caen en el cuantil 50%, por lo que estos resultados se deben interpretar con mucha cautela.       
...
# Conclusiones                  
  ...
  * Una proporcion baja de las focas muestreadas tuvieron piojos `r sum(leo$presence)/nrow(leo)*100`%.     
    * Esta proporcion fue casi identica en hembras (`r round(100*(t.lice.presence.sex %>% filter(sex == "F") %>% select(prop) %>% pull()), 1)`%), y en machos (`r round(100*(t.lice.presence.sex %>% filter(sex == "M") %>% select(prop) %>% pull()), 1)`%)         
     * Esta proporcion fue mas baja en adultos (`r round(100*(t.lice.presence.age_class %>% filter(age_class == "Adult") %>% select(prop) %>% pull()), 1)`%), que en juveniles (`r round(100*(t.lice.presence.age_class %>% filter(age_class == "Juvenile") %>% select(prop) %>% pull()), 1)`%)      
