...
No se pudo computar la condicion relativa de `r nrow(leo[is.na(rel_cond)])` focas, dado que no se registro su peso. De ellas, `r nrow(leo[is.na(rel_cond) & Lice > 0])` foca tenia `r pull(leo[is.na(rel_cond) & Lice > 0, .(Lice)])` piojos.            
